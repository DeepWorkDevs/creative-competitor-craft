...
const Index = () => {
  ...
  return <div className="min-h-screen flex flex-col pb-10">
      
      
      <main className="flex-1 container max-w-7xl px-4 sm:px-6 py-8">
        {!generatedImageUrl ? <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
        duration: 0.5
            ...
            <form onSubmit={handleGenerate}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <ImageUpload title="Competitor's App/Product" description="Upload a screenshot of your competitor's app or product" onImageUpload={setCompetitorImage} />
                
                <ProjectInputSection onImagesUpload={setProjectImages} onWebsiteUrlChange={setProjectWebsiteUrl} onDescriptionChange={setProjectDescription} />
              </div>
            ...
            duration: 0.4
                ...
                <Textarea id="customPrompt" placeholder="Add specific instructions for your ad creative (e.g., highlight unique features, target specific audience, include specific messaging)" className="mt-2 bg-background/50 resize-none" value={customPrompt} onChange={e => setCustomPrompt(e.target.value)} />
              </motion.div>
              
              {error && <div className="mb-6 p-4 rounded-lg border border-red-500/30 bg-red-500/10">
                  <p className="text-red-400">{error}</p>
                </div>}
              
              {isGenerating ? <div className="flex justify-center py-10">
                  {isAnalyzing ? <LoadingSpinner message="Analyzing images and creating marketing strategy..." /> : <LoadingSpinner message="Generating your ad creative..." />}
                </div> : <motion.div className="flex justify-center" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            duration: 0.4
                  ...
                  <Button type="submit" size="lg" disabled={!competitorImage || !projectImages.length && !projectWebsiteUrl && !projectDescription} className="bg-pirate-500 hover:bg-pirate-600 text-white font-medium px-8">
            ...
            delay: 0.2
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  {projectImages.length > 0 && <p>{projectImages.length} image{projectImages.length > 1 ? 's' : ''} uploaded</p>}
                  
                  {projectWebsiteUrl && <p>Website: {projectWebsiteUrl}</p>}
                  
                  {projectDescription && <p>Description provided</p>}
                  
                  {customPrompt && <div className="mt-3 pt-3 border-t border-white/10">
            ...
            delay: 0.4
                ...
                <Button variant="outline" onClick={resetForm} className="w-full bg-transparent border border-white/10 hover:bg-white/5">
                  Create Another Ad
                </Button>
              </motion.div>
            </div>
